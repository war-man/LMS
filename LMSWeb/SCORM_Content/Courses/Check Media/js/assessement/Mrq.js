   ...
   init (id, htmlNode, data, isReview = false, userAnswers = []) {
    ...
    createOption(data){
        this._htmlNode.find(".options").append($('<div/>', {
            ...
            'class': 'checkbox-button '+ this.reviewClass(), 'id':'checkbox-button-'+this._optionCount, 'href':'javascript:void(0)'
        }).append($('<span/>', {
    ...
    createReviewContent(){
        ...
        var f = this._isCorrect ? this._data.correctFeedback.text : this._data.incorrectFeedback.text
        //update feedback
        this._htmlNode.find(".feedback p").html(f);
    }
    reset(){
        this._htmlNode.find(".checkbox-button").removeClass("selected hover");
