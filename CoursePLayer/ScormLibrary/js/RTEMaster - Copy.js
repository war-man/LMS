...
function fResetError() {
	...
	function LMSIntSetValue(lParam, lData) {
	    debugPrint("Setvalue: lParam = '" + lParam + "', lData = '" + lData + "'");
	    
		//Modified the API by Rohit on 30th Oct 2009 for Indecomm Global Services LMS
...
//---------------------------------------------------------------------------
	...
	function fCheckAndCreateSCOBlock(){
		...
		if (!oSCO){
			...
			fSetDataFromManifest("cmi__student_data__mastery_score", "masteryscore"); // modified by Rohit(Indecomm Global Services)
...
//----------------------------------------------------------------------------
   ...
   function fSendRequest(lCase){      
	...
	{
	    if (!parent.ContentSrvFrame.controlFrame.document.formMain) {
