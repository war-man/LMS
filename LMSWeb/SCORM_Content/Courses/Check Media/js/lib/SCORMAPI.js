...
pipwerks.SCORM.connection.initialize = function () {
    ...
    if (!scorm.connection.isActive) {
        ...
        if (API) {
            ...
            if (success) {
				trace(traceMsgPrefix,"","",true,true);
...
pipwerks.SCORM.data.get = function (parameter) {
    ...
    if (scorm.connection.isActive) {
        ...
        if (API) {
            ...
            trace((scorm.version+'pipwerks.SCORM.data.get:'+ parameter+'value:' + value));
            errorCode = debug.getCode();
...
pipwerks.SCORM.data.set = function (parameter, value) {
    ...
    if (scorm.connection.isActive) {
        ...
        if (API) {
            ...
            trace((scorm.version+'pipwerks.SCORM.data.set:'+ parameter+'value:' + value+ 'result:' + success));
            if (success) {
...
pipwerks.SCORM.data.save = function () {
    ...
    if (scorm.connection.isActive) {
        ...
        if (API) {
            ...
            trace(scorm.version+ 'pipwerks.SCORM.data.save:'+ success);
        } else {
    ...
    if (success) {
		trace('SCORM.data.save',"","",'success:',true);
    } else {
		trace('SCORM.data.save',"","",'Failed:', false);
    }
    return success;
...
pipwerks.SCORM.debug.getCode = function () {
    ...
    if (API) {
        ...
        trace(scorm.version+ 'pipwerks.SCORM.debug.getCode:'+ code);
    } else {
...
pipwerks.SCORM.debug.getInfo = function (errorCode) {
    ...
    if (API) {
        ...
        trace(scorm.version+ 'pipwerks.SCORM.debug.getInfo:'+ result);
    } else {
...
function getScromErrorCodeInfo(val) {
	var str = "ErrorCode:" + val + ":";
	var is2004 = pipwerks.SCORM.version =="2004";
	switch (val) {
		case 0:
			str = "";/* str+="No error "; */
			break;
		case 101:
			...
			break;
		case 102:
			str += "General Initialization Failure ";
			break;
		case 103:
			str += "Already Initialized";
			break;
		case 104:
			str += "Content Instance Terminated";
			break;
		case 111:
			str += "General Termination Failure";
			break;
		case 112:
			str += "Termination Before Initialization";
			break;
		case 113:
			str += "Termination After Termination";
			break;
		case 122:
			str += "Retrieve Data Before Initialization";
			break;
		case 123:
			str += "Retrieve Data After Termination";
			break;
		case 132:
			str += "Store Data Before Initialization ";
			break;
		case 133:
			str += "Store Data After Termination ";
			break;
		case 142:
			str += "Commit Before Initialization ";
			break;
		case 143:
			str += "Commit After Termination ";
			break;
		case 201:
			str += "Invalid argument error ";
			break;
		case 201:
			str += "General Argument Error ";
			break;
		case 202:
			str += "Element cannot have children ";
			break;
		case 203:
			str += "Element not an array. Cannot have count";
			break;
		case 301:
			if (is2004) {
				str += "General Get Failure";
			} else {
				str += "Not initialized";
			}
			break;
		case 351:
			str += "General Set Failure";
			break;
		case 391:
			str += "General Commit Failure ";
			break;
		case 401:
			if (is2004) {
				str += "Undefined Data Model Element ";
			} else {
				str += "Not implemented error";
			}
			break;
		case 402:
			if (is2004) {
				str += "Unimplemented Data Model Element ";
			} else {
				str += "Invalid set value, element is a keyword ";
			}
			break;
		case 403:
			if (is2004) {
				str += "Data Model Element Value Not Initialized ";
			} else {
				str += "Element is read only 1.2 ";
			}
			break;
		case 404:
			if (is2004) {
				str += "Data Model Element Is Read Only";
			} else {
				str += "Element is write only1.2 ";
			}
			break;
		case 405:
			if (is2004) {
				str += "Data Model Element Is Write Only ";
			} else {
				str += "Incorrect Data Type 1.2";
			}
			break;
		case 406:
			str += "Data Model Element Type Mismatch ";
			break;
		case 407:
			str += "Data Model Element Value Out Of Range";
			break;
	}
	str += " ";
	return str;
